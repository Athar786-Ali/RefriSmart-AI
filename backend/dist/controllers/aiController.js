...
If isRelevant is false, leave other fields as empty strings.`;
        ...
        const fallbackCostRange = `₹${Number(fallback.estimatedCostMin).toLocaleString("en-IN")} - ₹${Number(fallback.estimatedCostMax).toLocaleString("en-IN")}`;
        if (!parsed) {
            parsed = {
                isRelevant: true,
                problem: fallback.probableFault || "an internal component fault",
                technicalExplanation: replyLanguage === "ENGLISH"
                ...
                safetyAlert: replyLanguage === "ENGLISH"
                ...
                conclusion: replyLanguage === "ENGLISH"
        ...
        const technicalExplanation = parsed.technicalExplanation ||
