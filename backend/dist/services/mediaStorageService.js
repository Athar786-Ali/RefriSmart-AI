...
const sanitizeFileName = (value) => value
    ...
    .replace(/-{2,}/g, "-")
    .replace(/^-+|-+$/g, "");
const getExtension = (mimeType, originalName, fallback = "bin") => {
...
const getResourceType = (mimeType, explicitType) => {
    if (explicitType)
        return explicitType;
...
const toLocalUrl = (folder, fileName) => `/uploads/${folder}/${fileName}`;
export const toAbsoluteMediaUrl = (req, rawUrl) => {
...
const persistLocallyFromBuffer = async (buffer, folder, mimeType, originalName) => {
...
const persistLocallyFromTempFile = async (input) => {
...
export const storeMediaFromDataUrl = async (input) => {
...
export const storeMediaFromTempFile = async (input) => {
...
export const removeStoredMedia = async (rawUrl) => {
    const value = String(rawUrl || "").trim();
    if (!value)
        return;
