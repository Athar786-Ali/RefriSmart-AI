...
export async function deliverLoginOtp(
  phone: string,   // 10-digit Indian number WITHOUT country code
  otp: string,
  ...
  if (AUTHKEY && WA_FLOW_ID) {
    try {
      ...
      console.warn(`[OTP] ⚠ WhatsApp failed (${(err as Error).message}), trying SMS…`);
    }
  }
  ...
  if (AUTHKEY && SMS_TMPL) {
    try {
      ...
      console.warn(`[OTP] ⚠ SMS failed (${(err as Error).message}), using dev fallback`);
    }
  }
  ...
  if (IS_PROD) {
    throw new Error(
      "OTP delivery failed. Set MSG91_AUTH_KEY + MSG91_TEMPLATE_ID in backend/.env"
    );
  }
  console.log(`\n╔══════════════════════════════════════╗`);
...
async function sendViaWhatsApp(mobile: string, otp: string): Promise<void> {
...
// ── MSG91 SMS OTP API ────────────────────────────────────────────────────
async function sendViaSms(mobile: string, otp: string): Promise<void> {
  const res = await fetchWithTimeout(
