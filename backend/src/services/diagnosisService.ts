...
export const ensureDiagnosisLogSchema = async () => {
  if (diagnosisSchemaEnsured) return;
...
export const createDiagnosisLog = async (input: DiagnosisLogInput) => {
...
export const listDiagnosisLogs = async (customerId?: string | null) => {
  await ensureDiagnosisLogSchema();

  if (customerId) {
    return prisma.$queryRaw<Array<DiagnosisLogRow>>`
      SELECT
        d."id",
        d."customerId",
        d."guestName",
