...
const ensurePhase2Schema = async () => {
  ...
  const existingTech = await prisma.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "Technician" LIMIT 1`;
...
const ORDER_STATUS_FLOW = ["ORDER_PLACED", "DISPATCHED", "OUT_FOR_DELIVERY", "DELIVERED"] as const;
...
type InputLanguage = "ENGLISH" | "HINGLISH" | "HINDI";
...
const fallbackStructuredDiagnosis = (appliance: string, issue: string, language: InputLanguage): StructuredDiagnosis => {
...
const generateSuggestedSlots = (startDate?: Date) => {
  const now = new Date();
...
const makeSimplePdfBuffer = (title: string, lines: string[]) => {
...
const generateFallbackDiagnosis = (appliance: string, issue: string, language: InputLanguage) => {
  const normalized = issue.toLowerCase();
...
app.post('/api/sell/request', async (req, res) => {
  try {
    ...
    const { userId, applianceType, brandModel, conditionNote, expectedPrice, pincode, imageUrl } = req.body as {
      userId?: string; applianceType?: string; brandModel?: string; conditionNote?: string; expectedPrice?: number | string; pincode?: string; imageUrl?: string;
    };
...
app.post('/api/sell/requests/:id/move-to-refurbished', async (req, res) => {
  try {
    ...
    const { sellerId, title, description, price, imageUrl } = req.body as {
      sellerId?: string; title?: string; description?: string; price?: number | string; imageUrl?: string;
    };
...
app.post('/api/docs/:docType', async (req, res) => {
  try {
    ...
    const { bookingId, amount, gst, signature, notes } = req.body as {
      bookingId?: string; amount?: number | string; gst?: number | string; signature?: string; notes?: string;
    };
    ...
    const now = new Date();
    const title =
      docType === "invoice" ? "Golden Refrigeration - Invoice" :
...
app.post('/api/auth/send-verify-otp', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    await ensureAuthSchema().catch(() => {});
...
app.post('/api/auth/verify-otp', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    await ensureAuthSchema().catch(() => {});
    const userId = req.userId;
    const { otp } = req.body as { otp?: string };
